  ...
  return (
    <div className="py-20">
      <div className="mx-auto lg:max-w-7xl lg:px-8 px-5">
        ...
        <Reveal>
          {/*carte */}
          <div className={`mx-auto mt-5 md:flex md:flex-wrap grid  gap-x-2 gap-y-10 sm:mt-16 lg:mx-0 lg:max-w-none  md:gap-4 md:p-1.5`}>
            {cards.map((card , i) => (
              <div key={i} className="flex-1 rounded-lg shadow-[0_0px_10px_rgba(150,150,150,0.15)] h-fit ">
                    <div className='flex flex-col  w-full h-full rounded-lg overflow-hidden gap-5'>
                    ...
                    <div className="h-50 w-full overflow-hidden">
                        ...
                        <div className="grid grid-cols-1 lg:px-5 px-2 md:gap-6 gap-3 ">
                            <p className='text-primary font-extrablack text-2xl md:text-xl lg:text-3xl h-15 '>{card.cardsTitre}</p>
                           {card.cardsDesc &&  <p className="md:h-20 text-neutral-500 font-light lg:text-[14px] text-sm">{card.cardsDesc}</p>}
                        </div>
                       ...
                       <Link to={card.cardsLink} className="flex items-center px-5 py-3 bg-primary text-white transition-all hover:text-white hover:bg-red-800 hover:gap-5 gap-2">
                           {card.cardsLinkLabel || t('menu.btn-contact')}
